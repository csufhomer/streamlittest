...
subscribers="Aggregated_Metrics_By_Country_And_Subscriber_Status.csv"
videoMetric="Aggregated_Metrics_By_Video.csv"
finalComments="All_Comments_Final.csv"
performance="Video_Performance_Over_Time.csv"

@st.cache_data
def load_data():
    #load data
    df_agg=pd.read_csv(videoMetric).iloc[1:,:]
    df_agg_sub=pd.read_csv(subscribers)
    df_comments=pd.read_csv(videoMetric)
    df_time=pd.read_csv(performance)
...
if add_sidebar == 'Aggregate Metrics':
    ...
    st.dataframe(df_agg_diff_final.style.hide().applymap(style_negative, 
         props='color:red;').applymap(style_positive, props='color:green;').format(df_to_pct))
...
#conda create -n stenv python=3.9    create a conda environment named stenv and python=3.9 
#conda activate stenv                Activate the conda environment 
#pip install streamlit               Install the Streamlit library 
#streamlit hello                     Launching the Streamlit demo app 
#Create a file called streamlit_app.py 
# run streamlit_app.py Fire up the command line terminal
...
#data set: https://www.youtube.com/redirect?event=video_description&redir_token=QUFFLUhqblZmS2ZNd1FxMEdKSVdXcjg3WXRLTTZncjFrUXxBQ3Jtc0ttdGRWNlFfRUo5ZDgtOWxXZzEwVW9tbkxVaWhkOWIwZk5yOXFkX3g5VExTMHhMRDRnWkxIS1hYdjE2R0hTck1aQnNRdWtaQ1FsQm9vUnd0ZW1aSUczWVFiRVo3WDdsZG12eWltd2FhUjF6bEVRSkJkYw&q=https%3A%2F%2Fwww.kaggle.com%2Fdatasets%2Fkenjee%2Fken-jee-youtube-data&v=Yk-unX4KnV4